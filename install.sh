...
IMAGE_BASE_PATH_VALUE="images"
IMAGE_PATH_VALUE="tmp"
...
if [ "${IFACE_CHOICE}" = "2" ]; then
    ...
    INDI_FLAG=1
    IMAGE_UPLOAD_SCRIPT="scripts.run_image_upload_indi_api"
    NIGHTLY_UPLOAD_SCRIPT="scripts.run_nightly_upload_indi_api"
    ...
    while true; do
        ...
        if [ "${ALLSKY_PATH}" = "${DEFAULT_ALLSKY_PATH}" ]; then
            INDI_IMAGES_DIR="${ALLSKY_PATH}/images"
            IMAGE_BASE_PATH_VALUE="images"
            echo "> Checking INDI standard images dir: ${INDI_IMAGES_DIR}"
        else
            INDI_IMAGES_DIR="${ALLSKY_PATH}"
            IMAGE_BASE_PATH_VALUE=""
            echo "> Checking custom INDI image dir: ${INDI_IMAGES_DIR}"
        fi
    ...
    if [ -f "${ALLSKY_PATH}/tmp/current_images/image.jpg" ] || \
       [ -f "${ALLSKY_PATH}/tmp/current_images/image.png" ]; then
        IMAGE_PATH_VALUE="tmp/current_images"
        echo "✅ Detected TJ live image path: ${IMAGE_PATH_VALUE}"
    elif [ -f "${ALLSKY_PATH}/tmp/image.jpg" ] || \
         [ -f "${ALLSKY_PATH}/tmp/image.png" ]; then
        IMAGE_PATH_VALUE="tmp"
        echo "✅ Detected TJ live image path: ${IMAGE_PATH_VALUE}"
    elif [ -d "${ALLSKY_PATH}/tmp/current_images" ]; then
        IMAGE_PATH_VALUE="tmp/current_images"
        echo "✅ Detected TJ directory path: ${IMAGE_PATH_VALUE}"
    elif [ -d "${ALLSKY_PATH}/tmp" ]; then
        IMAGE_PATH_VALUE="tmp"
        echo "✅ Detected TJ directory path: ${IMAGE_PATH_VALUE}"
    else
        IMAGE_PATH_VALUE="tmp/current_images"
        echo "⚠️ Could not detect TJ image path. Fallback: ${IMAGE_PATH_VALUE}"
    fi
fi
...
echo "IMAGE_BASE_PATH: ${IMAGE_BASE_PATH_VALUE}"
echo "IMAGE_PATH: ${IMAGE_PATH_VALUE}"
echo "Image upload script: ${IMAGE_UPLOAD_SCRIPT}"
echo "Nightly upload script: ${NIGHTLY_UPLOAD_SCRIPT}"
...
python3 -m pip uninstall -y Send2Trash send2trash 2>/dev/null || true
...
IMAGE_BASE_PATH = "${IMAGE_BASE_PATH_VALUE}"
IMAGE_PATH      = "${IMAGE_PATH_VALUE}"
IMAGE_UPLOAD_SCRIPT = "${IMAGE_UPLOAD_SCRIPT}"
NIGHTLY_UPLOAD_SCRIPT = "${NIGHTLY_UPLOAD_SCRIPT}"
INDI            = ${INDI_FLAG}
...
CRONTABS = [
    ...
    },
    {
        ...
        "schedule": "*/2 * * * *",
        "command": "cd ${PROJECT_ROOT} && python3 -m ${IMAGE_UPLOAD_SCRIPT}",
    },
    {
    ...
    },
    {
        ...
        "schedule": "45 8 * * *",
        "command": "cd ${PROJECT_ROOT} && python3 -m ${NIGHTLY_UPLOAD_SCRIPT}",
    },
    {
...
echo
echo "Support:"
echo "  Need help or have questions? Just reach out:"
echo "  gottie@web.de | +49 162 79 55 198"