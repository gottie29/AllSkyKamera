...
# Initial Jitter
NIGHTLY_UPLOAD_JITTER_MAX_SECONDS = int(getattr(config, "NIGHTLY_UPLOAD_JITTER_MAX_SECONDS", 600))
...
def _sanitize_ascii(s: str) -> str:
    # Sonderzeichen entfernen (tail -f freundlich)
    try:
...
def _log_nightly_status(value: int, asset: str):
    try:
        ...
        # darf nie den Upload beeinflussen
        pass
...
def _apply_initial_jitter():
    max_s = int(NIGHTLY_UPLOAD_JITTER_MAX_SECONDS)
    if max_s <= 0:
        return
    delay = random.randint(0, max_s)
    if delay > 0:
        log(f"nightly_jitter_seconds={delay}")
        time.sleep(delay)
...
def upload_nightly_batch(date_str: str = None) -> bool:
    ...
    if not indi_flag:
        ...
        if video: files.append(("video", video, config.FTP_VIDEO_DIR, None))
        if keo:   files.append(("keogram", keo, config.FTP_KEOGRAM_DIR, f"keogram-{date_str}.jpg"))
        if st:    files.append(("startrail", st, config.FTP_STARTRAIL_DIR, f"startrails-{date_str}.jpg"))
        if a_used:
            files.append(("analemma_used", a_used, config.FTP_ANALEMMA_DIR, os.path.basename(a_used).replace(".png", ".jpg")))
        if a_unused:
            files.append(("analemma_unused", a_unused, config.FTP_ANALEMMA_DIR, os.path.basename(a_unused).replace(".png", ".jpg")))
        ...
        if tl:  files.append(("video", tl, config.FTP_VIDEO_DIR, None))
        if keo: files.append(("keogram", keo, config.FTP_KEOGRAM_DIR, f"keogram-{date_str}.jpg"))
        if st:  files.append(("startrail", st, config.FTP_STARTRAIL_DIR, f"startrails-{date_str}.jpg"))
        if stv: files.append(("startrail_video", stv, startrailsvideo_dir, None))
            ...
            for asset, local_path, remote_subdir, forced_remote_name in files:
                ...
                while True:
                    ...
                    try:
                        ...
                        is_startrail_video = lp.endswith((".mp4", ".webm")) and "startrail_timelapse" in lp and indi_flag
