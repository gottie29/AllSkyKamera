...
def _parse_date_arg(arg: str) -> str:
    _log(f"Prüfe Datumsargument: {arg}")
    ...
    _log(f"Datumsargument gültig: {normalized}")
    return normalized
...
def _show_paths(date_str: str | None) -> None:
    """
    ...
    try:
        ...
        _log(f"Pfade konnten nicht geprüft werden: {e}")

    _log("-----------------------------")


def main() -> None:
    _log("Starte run_nightly_upload_api")
    ...
    if len(sys.argv) == 2:
        ...
        _log(f"Manueller Upload für Datum {date_str}")
    else:
        _log("Kein Datum übergeben -> automatische Datumslogik")
